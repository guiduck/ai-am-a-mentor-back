    ...
    return new Promise((resolve) => {
      ffmpeg(videoPath)
        .outputOptions([
          ...
          "-ar",
          "16000", // Sample rate (16kHz is sufficient for speech)
          "-ac",
          "1", // Mono (reduces size)
          "-b:a",
          "64k", // Audio bitrate (64kbps is sufficient for speech)
        ])
        .output(audioPath)
...
export async function extractAudioFromVideoStream(
  ...
  try {
    ...
    writeStream = createWriteStream(videoPath);
  ...
  return new Promise((resolve) => {
    ...
    ffmpeg.getAvailableEncoders((err, encoders) => {
      if (err) {
        ...
        console.log(
          "✅ FFmpeg is available, encoders found:",
          Object.keys(encoders || {}).length
        );
        resolve(true);
      }
    });
